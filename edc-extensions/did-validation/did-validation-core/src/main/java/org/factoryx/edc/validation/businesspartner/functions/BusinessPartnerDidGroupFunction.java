 ...
 * The {@link ParticipantAgent} is represented by its DID, the {@link Operator} and the {@code rightValue} determine the group(s) and
 * whether the DID must part of it, or not be part of it.
 * <p>
 * For example, a {@link org.eclipse.edc.policy.model.Policy} that mandates the DID be part of a group {@code "gold_customers"} or {@code "platin_partner} could look like this:
 *
 * <pre>
 * {
 *     "constraint": {
 ...
 * </pre>
 * <p>
 * Upon evaluation, the {@link BusinessPartnerDidGroupFunction} will take the {@link ParticipantAgent}s DID, use it to resolve the groups that the DID is part of, and check, whether `"gold_partner"` and
 ...
 */
public class BusinessPartnerDidGroupFunction<C extends ParticipantAgentPolicyContext> implements AtomicConstraintRuleFunction<Permission, C> {
    /**
     * Key used in policy constraints to identify business partner group constraints.
     */
    ...
    /**
     * Constructs a new BusinessPartnerDidGroupFunction.
     *
     * @param store the business partner store for resolving DID groups
     * @param monitor the monitor for logging
     */
    public BusinessPartnerDidGroupFunction(BusinessPartnerStore store, Monitor monitor) {
        this.store = store;
    ...
    public boolean evaluate(Operator operator, Object rightOperand, Permission permission, C context) {
        ...
        var did = participantAgent.getIdentity();
        var groups = store.resolveForBpn(did);

        var assignedGroups = groups.getContent();

        // DID not found in database
        if (groups.failed()) {
    ...
    private boolean evaluateIsAnyOf(DidGroupHolder didGroupHolder) {
        ...
        var allowedGroups = didGroupHolder.allowedGroups;
        return didGroupHolder.assignedGroups
                .stream()
